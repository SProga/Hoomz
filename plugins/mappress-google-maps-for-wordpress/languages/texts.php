<?php
__("MapPress Maps for WordPress");
__("https://www.mappresspro.com");
__("MapPress makes it easy to add Google and Leaflet Maps to WordPress");
__("Chris Richardson");
__("Settings");
__("Maps");
__("Import");
__("Support");
__("MapPress database tables are missing.  Please deactivate the plugin and activate it again to fix this.");
__("A Google Maps API key is required");
__("Please update your");
__("MapPress Settings");
__("Renew your license");
__("Your MapPress license has expired.  %s to get the latest updates and prevent errors.");
__("Please enter your MapPress license key to enable plugin updates");
__("MapPress Gutenberg blocks require WordPress 5.3 or the latest Gutenberg Plugin. Please update if using the block editor.");
__("MapPress has many new features!  %s.");
__("Learn more");
__("MapPress popup templates have changed!  Please update custom templates to the new format. %s.");
__("MapPress templates and filters have changed.  Please update custom templates and filters. %s.");
__("Are you sure you want to delete?");
__("Permanently delete this map?");
__("Error reading KML file");
__("URL for KML file");
__("Please select an editor to insert into.");
__("Unable to get your location");
__("No results");
__("Save changes?");
__("Shape");
__("Streets");
__("Outdoors");
__("Light");
__("Dark");
__("Satellite");
__("Satellite Streets");
__("Roadmap");
__("Terrain");
__("Hybrid");
__("What's new");
__("Version");
__("Map not found");
__("Post not found");
__("Untitled");
__("Copy of %s");
__("MapPress");
__("Invalid lat/lng coordinate: %s,%s");
__("Default");
__("Address line ");
__("Latitude");
__("Longitude");
__("Title");
__("Body");
__("Icon");
__("Zoom");
__("OK, you deserve it!");
__("Nope, maybe later");
__("I need help using the plugin");
__("Help Spread the Word");
__("Hi, I hope you're enjoying MapPress.  Would you mind taking a moment to write a brief review?  It would mean a lot to me!");
__("Get help");
__("Your account");
__("Upgrade to MapPress Pro");
__("Address");
__("Thumbnail");
__("Url");
__("Custom Field");
__("Results");
__("%d of %d");
__("Get Directions");
__("Your Location");
__("List");
__("Map");
__("Continue reading");
__("By %s");
__("In");
__(" Tagged ");
__("Visual");
__("HTML");
__("Add Media");
__("Save");
__("Cancel");
__("Delete");
__("My location");
__("All");
__("Filter");
__("Reset");
__("Done");
__("Search");
__("Enter an address, place, KML file URL, or lat,lng");
__("Traffic");
__("Bicycling");
__("Transit");
__("Center map");
__("Help");
__("Feedback");
__("contact me");
__("Sorry to see you go!  Need help?");
__("Contact me");
__("If not, I'd really appreciate some feedback:");
__("I'm just having a bad day");
__("The plugin isn't working");
__("What didn't work?  Was there an error message?");
__("Too hard to use");
__("How can I make it easier?");
__("Missing feature");
__("What's missing?");
__("Other");
__("Can you provide any details?");
__("Deactivate");
__("No posts found");
__("Attach");
__("Please select the post content to insert the map into.");
__("Select a color");
__("Opacity");
__("Stroke");
__("Custom color");
__("Map title");
__("Map ID");
__("No maps found");
__("Open map library");
__("Select icon");
__("New icon");
__("Custom icons");
__("Standard icons");
__("Use default icon");
__("Edit list");
__("No icon selected");
__("Select icons to delete");
__("Create new icon");
__("Font");
__("Font size");
__("Color");
__("Fill");
__("Upload icons");
__("Source");
__("Google Sheets");
__("File");
__("Google sheets must be shared before importing:");
__("File -> Share -> Publish to web");
__("Select 'Entire Document' and 'Comma-Separated Values (.csv)");
__("Copy the URL and paste it here");
__("File must be in CSV format:");
__("Excel: File -> Save As -> 'CSV' or 'CSV UTF-8' (for special characters)'");
__("Google sheets: File -> Download -> Comma Separated Values (.csv)");
__("Select file");
__("Use map IDs from file");
__("WARNING: this will overwrite any existing maps with the same IDs");
__("Line");
__("Result");
__("Show map");
__("Edit map");
__("Map Library");
__("MapPress Map");
__("Use the map library to create a new map or insert an existing map.");
__("Insert a map");
__("Maps for WordPress");
__("Sample Map");
__("License");
__("MapPress license key");
__("Enter license to enable automatic updates");
__("Get license");
__("Check now");
__("Active");
__("License is invalid or expired");
__("Beta versions");
__("Enable updates for beta versions");
__("Basic Settings");
__("Mapping API");
__("Leaflet is free and requires no API key.  Google requires an API key and has usage limits.");
__("Nominatim is free but inaccurate.  Consider using Mapbox or Google instead.");
__("Geocoder");
__("Get an Access Token");
__("Mapbox access token");
__("Enter token to use Mapbox");
__("Mapbox makes Leaflet maps look great and provides an excellent geocoder.");
__("Get an API Key");
__("Google API key");
__("Enter API key to use Google maps");
__("Enter API key to use Google geocoder");
__("Map Settings");
__("Automatic display");
__("None");
__("Top of post");
__("Bottom of post");
__("Map alignment");
__("Center");
__("Left");
__("Right");
__("Directions");
__("Inline");
__("Mini width");
__("Width (px) to use map/list toggle buttons");
__("Scroll wheel zoom");
__("Enable map zoom with mouse scroll wheel");
__("POIs");
__("Default zoom");
__("Default zoom when displaying a single POI");
__("Hover");
__("Open POIs on hover");
__("Open first POI");
__("Open first POI when a map is displayed");
__("Travel lines");
__("Connect POIs with lines");
__("Circle");
__("Arrow");
__("Open arrow");
__("Geodesic (curved) lines");
__("Animate icon");
__("Travel line options");
__("POI List");
__("Display");
__("Show a list of POIs with each map");
__("Position");
__("Left of map");
__("Below map");
__("Initial open");
__("Open list when map is displayed");
__("Paging");
__("POIs per page");
__("Sort");
__("Sort POI list by title");
__("Styled Maps");
__("Default style");
__("Replace the default \"roadmap\" or \"streets\" style");
__("Icons");
__("Default icon");
__("Icon directory");
__("Highlight selected POI");
__("Scale");
__("Icon size");
__("Post type");
__("Automatic icons");
__("Be sure to set \"Show in REST API\" for custom taxonomies");
__("Key");
__("Clustering");
__("Enable marker clustering");
__("Max zoom");
__("Disable clustering at zoom");
__("Auto");
__("Spiderfy");
__("Mashups");
__("Open popup");
__("Open post");
__("Post in new tab");
__("Post in modal");
__("Marker click");
__("Popup content");
__("POI title + POI body");
__("Post title + post excerpt");
__("KMLs");
__("Include KML POIs in mashups");
__("Enable search for mashups");
__("Search radius");
__("Minimum search radius (km)");
__("Search country");
__("Country code for searches");
__("User location");
__("Show user location on map");
__("Bounding box");
__("Restrict searches to a bounding box (optional)");
__("Mashup filters");
__("Filters position");
__("Top");
__("POI list");
__("Filters");
__("Text search");
__("New filter");
__("Input");
__("Checkboxes");
__("Radio buttons");
__("Select");
__("Filter type");
__("Format");
__("Label");
__("Name");
__("Slug");
__("Include");
__("Exclude");
__("Templates");
__("Custom templates");
__("Mashup thumbnails");
__("Popups");
__("POI lists");
__("Thumbnail size");
__("or");
__("Localization");
__("Language");
__("Language for searches and Google map controls");
__("Directions server");
__("Generate Maps from Custom Fields");
__("Custom fields");
__("Map field");
__("Custom field");
__("Post types");
__("Overwrite");
__("Overwrite maps when posts are saved");
__("Google Server API key");
__("Server API key for geocoding.  Only required if client key is restricted by referrer.");
__("Frontend Forms");
__("Advanced Custom Fields");
__("ACF field");
__("ACF map field to include on MapPress mashups");
__("To generate MapPress maps from ACF fields, leave this blank and use the Geocoding section");
__("Miscellaneous");
__("Map sizes");
__("Width");
__("Height");
__("Scripts");
__("Output scripts in footer");
__("Disable footer scripts if maps are output using AJAX");
__("Copy maps when overwriting a translation");
__("Compatibility");
__("Prevent loading Google Maps multiple times");
__("Output maps in iframes to prevent compatibility issues");
__("Reset Defaults");
__("Add");
__("Custom");
__("New");
__("Map popup");
__("Map list item");
__("Mashup popup");
__("Mashup list item");
__("Editor");
__("Geocoding");
__("Geocode now");
__("New posts will be geocoded when they are published.");
__("Use an online geocoding service for bulk geocoding (>100 items).");
__("No errors");
__("Geocoding errors");
__("Current post");
__("Edit post");
__("Unattached");
__("Map restored");
__("Restore");
__("Map deleted");
__("Delete Permanently");
__("Edit");
__("Insert into post");
__("Map copied");
__("Copy");
__("Map moved to trash");
__("Trash");
__("Detach");
__("Shortcode copied");
__("Add New");
__("Attached to post");
__("post title, map title, or map id");
__("ID");
__("Attached to");
__("Shortcode");
__("Map %d saved");
__("Map settings");
__("Style");
__("Size");
__("px, %, vw");
__("px, %, vh");
__("Center/Zoom");
__("Viewport automatic");
__("Automatic");
__("Viewport set");
__("Set");
__("Style already exists, overwrite?");
__("Please upgrade to MapPress Pro to use custom styles");
__("Please enter a Mapbox access token in the MapPress settings screen to use styled maps.");
__("Add style");
__("Enter style from Mapbox Studio");
__("MapBox Share URL");
__("Style name");
__("Custom styles");
__("Select a style");
__("Standard styles");
__("New style");
__("Download style");
__("Enter style name");
__("JSON");
__("MapPress Support");
__("Build amazing maps with the easiest and most powerful mapping plugin available");
__("Open Setup Wizard");
__("Documentation");
__("Detailed plugin documentation:");
__("Creating your first map");
__("Troubleshooting");
__("Check for a plugin or theme conflict:");
__("Deactivate ALL other plugins");
__("Switch to a standard WordPress theme");
__("If the maps are working, reactivate one by one to find the conflict.");
__("For more info:");
__("MapPress FAQ");
__("Need some help?");
__("Support forum");
__("Contact");
__("Please Choose a Mapping API");
__("MapPress supports both Leaflet and Google mapping APIs.");
__("No API key");
__("Free unlimited usage");
__("Good functionality");
__("Best for high traffic sites");
__("API key required");
__("Limited free usage");
__("Excellent functionality");
__("Best for low traffic sites");
__("Welcome to MapPress!");
__("Google API Key");
__("Google requires an API key to use Google Maps.  Please enter your key below.");
__("Enter your API key here");
__("Sign up with Mapbox");
__("Mapbox makes Leaflet better with great-looking map tiles, styled maps, and a powerful geocoder.");
__("A generous free tier means it's free for most sites.");
__("Get Mapbox Access Token");
__("Enter your access token here");
__("Maps in the Block Editor");
__("MapPress provides custom block types for the Gutenberg editor.");
__("Just insert a MapPress Map block to get started.");
__("Maps in the Classic Editor");
__("MapPress also supports the \"classic\" editor.  Just click the MapPress button to open the map library.");
__("The Map Library provides an easy way to see and edit all of your maps.  The library can be opened in the post editor or from the MapPress menu.");
__("Happy Mapping");
__("Enjoy the plugin!  Replay this guide any time from the MapPress support menu.");
?>